#include "multi_object_tracker/tracker_node.hpp"
...
TrackerNode::TrackerNode(const rclcpp::NodeOptions & options)
  : Node("multi_object_tracker_node", options), marker_id_(0) {
  // Initialize TF2
...
void TrackerNode::publishMarkers(const std_msgs::msg::Header& header) {
  ...
  for (const auto& track : tracks) {
    ...
    auto color_it = colors.find(track->state);
    if (color_it != colors.end()) {
      cube_marker.color.r = color_it->second[0];
      cube_marker.color.g = color_it->second[1];
      cube_marker.color.b = color_it->second[2];
    } else {
    ...
    cube_marker.lifetime.nanosec = 100000000;  // 0.1 seconds
...
RCLCPP_COMPONENTS_REGISTER_NODE(multi_object_tracker::TrackerNode)